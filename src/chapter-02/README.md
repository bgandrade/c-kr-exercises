...
- [x] `2-02` - Write a loop equivalent to the `for` loop above without using `&&` or `||`.
- [x] `2-03` - Write the function `htoi(s)`, which converts a string of hexadecimal digits, including an optional `0x` or `0X`, into its equivalent integer value.
- [x] `2-04` - Write an alternate version of `squeeze(s1, s2)` that deletes each character in `s1` that matches any character in the string `s2`.
- [x] `2-05` - Write the function `any(s1, s2)`, which returns the first location in the string `s1` where any character from the string `s2` occurs, or `-1` if `s1` contains no characters from `s2`.
